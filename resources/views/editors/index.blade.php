@extends('layouts.master')
@section('title','Editors')
@section('content')

<div class="container mt-5">
<div class="row">

@forelse($editors as $editor)

<div class="col-md-3">
<div class="card-deck">

  <div class="card my-4">
 
    <img  class="img-fluid"src="data:image/jpeg;base64," class="card-img-top" alt="...">
    <a href="{{route('editors.edit',['editor'=>$editor->id])}}" class="float-right"><i class="fa fa-edit text-dark"></i></a>
    <div class="card-body">
    
      <h5 class="card-title">Name-{{$editor->name}} (<small>{{$editor->role->name}}</small>)</h5>
     @if($editor->role->name == "editor")
     <a href="{{route('editors.destroy',$editor->id)}}" class="btn btn-dark btn-sm text-warning float-left">Delete &nbsp;&nbsp;&nbsp;<i class="fa fa-trash text-light"></i></a>

      <a href="{{route('promote',['id'=>$editor->id])}}" class="btn btn-dark text-warning btn-sm float-right">Promote &nbsp;&nbsp;&nbsp;<i class="fa fa-angle-double-up text-light"></i></a>
    @endif
    </div>
  </div>
</div>
</div>
@empty
<div class="jumbotron container bg-warning text-center my-5">
 <h1 class="display-4">No Data Avaiable Now..!</h1>
 </div>
 @endforelse
 </div>
 </div>

<a href="{{route('trash')}}" class="btn btn-md btn-dark text-warning float-right mr-5">View Trash &nbsp; <i class="fa fa-trash text-light"></i></a>


@endsection
